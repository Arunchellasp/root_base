// ============================================================
// gts.h - Primary GoogolTech motion control API declarations.
// Link requirements: gts.lib (or gts_x64.lib) + gts.dll at runtime.
//
// Use guide:
// 1. Include this header in files that call GT_* motion control functions.
// 2. Link the matching vendor import library for your target architecture.
// 3. Keep gts.dll beside the executable or in the process DLL search path.
// 4. Call GT_Open(), GT_Reset(), and GT_LoadConfig() before axis or motion commands.
// 5. Clear status/position when required, enable axes, configure motion, then start motion.
// 6. Poll status and check every return value; 0 means success, non-zero means API error.
// 7. Stop active motion and call GT_Close() before application exit.
// ============================================================
...
#define CRD_OPERATION_TYPE_BUF_FOLLOW_EVENT_TRIGGER	 (62)	// New look-ahead commands need these macros for distinction. GT_BufFollowEventCross and GT_BufFollowEventTrigger share CRD_OPERATION_TYPE_BUF_FOLLOW_EVENT, so this macro distinguishes them.
...
#define CRD_OPERATION_TYPE_WAIT_VAR_LONG   		 (131)// GSN macro is 31; conflicts with GTS, changed to 131
#define CRD_OPERATION_TYPE_WAIT_DI               (132)// GSN macro is 32; conflicts with GTS, changed to 132
...
typedef struct 
{
	long segVelType;             // Velocity profile type
	long endVelType;             // Last velocity profile
	double segTime;              // Motion time of the whole segment
	double segAccTime;           // Acceleration segment time
	double segEvenTime;          // Constant-velocity segment time
	double segDccTime;           // Deceleration segment time
	double segAccLength;         // Acceleration segment distance
	double segEvenLength;        // Constant-velocity segment distance
	double segDccLength;         // Deceleration segment distance
	double segMaxVel;            // Maximum velocity
	double segStartVel;          // Start velocity
	double segEndVel;            // End velocity
}TVelInfo;

typedef struct CrdPrm
{
    short dimension;                              // Coordinate system dimension
    short profile[8];                             // Associated profiles and coordinate axes
    double synVelMax;                             // Maximum composite velocity
    double synAccMax;                             // Maximum composite acceleration
    short evenTime;                               // Minimum constant-velocity time
    short setOriginFlag;                          // Flag for setting origin coordinate value, 0: default current planned position as origin; 1: user-specified origin position
    long originPos[8];                            // User-specified origin position
}TCrdPrm;

typedef struct CrdBufOperation
{
    short flag;                                   // Indicates whether this interpolation segment contains IO and delay
    unsigned short delay;                         // Delay time
    short doType;                                 // Buffer IO type, 0: no IO output
    unsigned short doMask;                        // Buffer IO output control mask
    unsigned short doValue;                       // Buffer IO output value
    unsigned short dataExt[CRD_OPERATION_DATA_EXT_MAX];     // Auxiliary operation extension data
}TCrdBufOperation;

typedef struct CrdData
{
    short motionType;                             // Motion type, 0: linear interpolation, 1: circular interpolation
    short circlePlat;                             // Circular interpolation plane
    long pos[8];                                  // End position of each axis in the current segment
    double radius;                                // Radius for circular interpolation
    short circleDir;                              // Circular rotation direction, 0: clockwise; 1: counterclockwise
    double center[3];                             // Center coordinates for circular interpolation
    double vel;                                   // Composite target velocity of the current segment
    double acc;                                   // Composite acceleration of the current segment
    short velEndZero;                             // Indicates whether the current segment end velocity is forced to 0, 0: not forced; 1: forced
    TCrdBufOperation operation;                   // Buffer delay and IO structure

    double cos[8];                                // Cosine value corresponding to each axis in the current segment
    double velEnd;                                // Composite end velocity of the current segment
    double velEndAdjust;                          // Variable used when adjusting end velocity (look-ahead module)
    double r;                                     // Composite displacement of the current segment
}TCrdData;
...
typedef struct
{
	short          mdlType;                       // Extension module type

	short          mdlAddressBegin;               // Start address offset of this module

	short          adChannels;                    // Number of channels when AD
	double         adMaxVoltage;                  // Voltage value corresponding to maximum conversion bit when AD
	double         adMinVoltage;                  // Voltage value corresponding to minimum conversion bit when AD

	short          daChannels;                    // Number of channels when DA
	double         daMaxVoltage;                  // Voltage value corresponding to maximum conversion bit when DA
	double         daMinVoltage;                  // Voltage value corresponding to minimum conversion bit when DA

	unsigned short outSave;                       // Retain current IO output value
}TExtMdlCfgInfo;
...
typedef struct  
{
	short count[2];                               // Row and column count, minimum value is 2
	long posBegin[2];                             // Start position
	long step[2];                                 // Step length
} TCompensate2DTable;

typedef struct  
{
	short enable;                                  // 2D compensation enable flag
	short tableIndex;                              // 2D compensation table in use
	short axisType[2];	                           // Lookup-table axis type
	short axisIndex[2];	                           // Lookup-table axis index
} TCompensate2D;
...
typedef struct  
{
	short mode;						// Homing mode
	short moveDir;					// Motion direction when starting the search
	short indexDir;					// Index search direction
	short edge;						// Capture edge setting
	short triggerIndex;				// Trigger setting; -1 means corresponding to the axis number
	short pad1[3];					// Reserved 1
	double velHigh;					// Home search velocity
	double velLow;					// Index search velocity
	double acc;					
	double dec;
	short smoothTime;
	short pad2[3];					// Reserved 2
	long homeOffset;				// Origin offset
	long searchHomeDistance;		// Maximum Home search distance; 0 means unlimited
	long searchIndexDistance;		// Maximum Index search distance; 0 means unlimited
	long escapeStep;				// Escape step length
	long pad3[2];					// Reserved 3
} THomePrm;
...
{
	short refType;			// Output reference type, only MC_AXIS is valid
	short refIndex;			// Output axis number
	short feedbackType;		// Feedback type; MC_ENCODER means encoder feedback, MC_ADC means voltage feedback
	short feedbackIndex;	// Feedback channel number
	long errorLimit;		// Error limit, currently not effective
	short feedbackSmooth;	// Feedback smoothing coefficient, currently not effective
	short controlSmooth;	// Controller smoothing coefficient, currently not effective
...
typedef struct AdcConfig
{
	short active;	
	short reverse;
	double a;			// Input voltage linear transform value a, value is 1
	double b;			// Input voltage linear transform value b, value is 0
	short filterMode;	// Filter mode, 0-FIR filter mode, 1-IIR filter mode
...
//Pci Interrupt
//////////////////////////////////////////////////////////////////////////
#define PCI_INTERRUPT_SOURCE_USER		(1)				// PCI interrupt source is user-defined
#define PCI_INTERRUPT_SOURCE_2DCOMPARE	(2)				// PCI interrupt source is 2D position comparison
typedef struct
{
...
typedef struct
{
	short work;			// Whether parameters have already been set
	short n;			// Number of compensation points
	long startPos;		// Start position
	long lenPos;		// Total length
	long compPos[256];	// Positive-direction compensation value
	long compNeg[256];	// Negative-direction compensation value
...
typedef struct  
{
	...
	short y;
	short z;
	double alpha;		// Angle between Z axis and XY plane
	double beta;		// Angle between the projection of Z axis on the XY plane and the X axis
	double gama;		// Angle between X and Y axes
...
typedef struct  
{
	short x;						// Planning axis corresponding to X axis
	short y;						// Planning axis corresponding to Y axis
	short c;						// Planning axis corresponding to C axis

	short tableRadiusIndex;			// Radius compensation table index
	short tableAngleIndex;			// C-axis compensation table index

	short directionReverse;			// By default, the angle also increases when the planned position increases

	long offset;		            // Planned position when C-axis rotation angle is 0
	long resolution;				// Pulses per C-axis revolution

	short adcIndex;					// ADC index
	double adcThreshold;			// ADC trigger compensation threshold
} TSmartCutterPrm;
...
#define STANDARD_HOME_STAGE_IDLE  (0) // Homing not started
#define STANDARD_HOME_STAGE_START (1) // Homing started
#define STANDARD_HOME_STAGE_SEARCH_HOME (20) // Searching Home
#define STANDARD_HOME_STAGE_SEARCH_INDEX  (30) // Searching Index
#define STANDARD_HOME_STAGE_GO_HOME       (80) // Moving to origin
#define STANDARD_HOME_STAGE_END           (100) // Homing ended
#define STANDARD_HOME_STAGE_START_CHECK (-1) // Self-check before homing starts
#define STANDARD_HOME_STAGE_CHECKING (-2) // Self-checking


#define STANDARD_HOME_ERROR_NONE		(0) // No error occurred
#define STANDARD_HOME_ERROR_DISABLE	(10) // Axis performing homing is not enabled
#define STANDARD_HOME_ERROR_ALARM		(20) // Axis performing homing has an alarm
#define STANDARD_HOME_ERROR_STOP		(30) // Homing was not completed and motion was stopped
#define STANDARD_HOME_ERROR_ON_LIMIT   (40) // Limit was triggered and cannot continue
#define STANDARD_HOME_ERROR_NO_HOME	(50) // Home not found
#define STANDARD_HOME_ERROR_NO_INDEX  (60) // Index not found
#define STANDARD_HOME_ERROR_NO_LIMIT (70) // Limit not found
#define STANDARD_HOME_ERROR_ENCODER_DIR_SCALE (-1) // Planner and encoder directions are opposite or pulse equivalents are inconsistent


typedef struct  
{
	short mode;		 // Homing mode, value range 1~36
	double highSpeed; // Home search velocity, unit: pulse/ms
	double lowSpeed;	 // Index search velocity, unit: pulse/ms
	double acc;		 // Homing acceleration, unit: pulse/ms^2
	long offset;       // Homing offset, unit: pulse
	short check; // Whether to enable self-check, 1-enabled, other values-disabled
	short autoZeroPos; // Whether to clear position automatically after homing, 1-clear automatically, other values-do not clear
	long motorStopDelay; // Motor in-position delay, unit: control period
	short pad1[3];	 // Reserved (does not need to be set)
} TStandardHomePrm;

typedef struct  
{
	short run;     // Whether homing is in progress, 0-motion stopped, 1-homing
	short stage;   // Homing stage
	short error;    // Error that occurred during homing
	short pad1[3];       // Reserved (no specific meaning)
	long capturePos;  // Encoder position at the moment Home or Index is captured
	long targetPos;    // Target position to move to (origin or origin + offset). During Limit/Home/Index search, if search distance is 0, this value displays as 805306368
} TStandardHomeStatus;
...
//////////////////////////////////////////////////////////////////////////
// Position compare output compatible with GSN position compare commands
//////////////////////////////////////////////////////////////////////////
typedef struct
{
	short mode;                       // FIFO mode; Linear mode
	short dimension;                  // 1D,2D
	short sourceMode;                 // Encoder, pulse counter
	short sourceX;                    // X-axis compare source
	short sourceY;                    // Y-axis compare source
	short outputMode;                 // Output mode: pulse, level
	short outputCounter;              // Output pulse count source
	unsigned short outputPulseWidth;  // Output pulse width; invalid in level mode
	unsigned short errorBand;         // 2D position compare output error band
} TPosCompareMode;


typedef struct
{
	short mode;                       // 0: time mode, 1: pulse mode
	unsigned short count;             // Output count
	short highLevel;// In pulse mode, inactive level duration; in level mode, high-level duration
	short lowLevel;// In level mode, low-level duration; invalid in pulse mode
	short resver[20];// Reserved
...
typedef struct
{
	unsigned short mode;      // 0: FIFO mode; 1: Linear mode
	unsigned short run;
...
//////////////////////////////////////////////////////////////////////////
// Hand joystick - Zhaofeng
//////////////////////////////////////////////////////////////////////////
#define  MC_JOYSTICK_AXIS_COUNT_MAX 3// Maximum joystick axis count
#define  MC_JOYSTICK_VEL_COUNT_MAX 3// Maximum velocity level count

typedef struct
{
	long scale[MC_JOYSTICK_AXIS_COUNT_MAX];// Pulse equivalent: pulse/mm
	double motionAcc[MC_JOYSTICK_AXIS_COUNT_MAX];// Acceleration: mm/s^2
	double motionDec[MC_JOYSTICK_AXIS_COUNT_MAX];// Deceleration: mm/s^2
	double motionSmooth[MC_JOYSTICK_AXIS_COUNT_MAX];// Smoothing coefficient: 0-1
}TJoystickMoitonPrm;

typedef struct
{
	double xMotionVel[MC_JOYSTICK_VEL_COUNT_MAX];// X velocity: mm/s
	double yMotionVel[MC_JOYSTICK_VEL_COUNT_MAX];// Y velocity: mm/s
	double zMotionVel[MC_JOYSTICK_VEL_COUNT_MAX];// Z velocity: mm/s
}TJoystickMoitonVel;


typedef struct
{
	long thresholdValue[MC_JOYSTICK_AXIS_COUNT_MAX];// Joystick threshold
	long joystickMaxValue[MC_JOYSTICK_AXIS_COUNT_MAX];// Maximum value
	long joystickMedianValue[MC_JOYSTICK_AXIS_COUNT_MAX];// Median value
	long joystickMinValue[MC_JOYSTICK_AXIS_COUNT_MAX];// Minimum value
...
GT_API GT_JoystickEnable(short joystickIndex,short enable);// mode 0: disabled, 1: enabled
...
GT_API GT_JoystickCheckEnable(short joystickIndex,short checkEnable);// checkEnable 0: enable validation, 1: validation disabled
GT_API GT_GetJoystickSts(short joystickIndex,long *pSts);// pSts bit0: joystick status (0: disabled, 1: enabled), bit1: joystick validation status (0: validation enabled, 1: validation disabled)
...
//typedef struct
//{
//	double pos[ELLIPSE_AUX_POINT_COUNT][8];    // Auxiliary point coordinates on the ellipse
//} TEllipseAuxPoint;
//
//typedef struct  
//{
//	double centerPoint[8];                     // Ellipse center coordinates
//	double theta;                              // Ellipse rotation angle
//	double a;                                  // Ellipse major axis
//	double b;                                  // Ellipse minor axis
//}TEllipseStandard;
//
//typedef union
//{
//	TEllipseAuxPoint auxPoint;                 // Auxiliary point mode parameters
//	TEllipseStandard standard;                 // Standard mode parameters
//	double reserve[60];
//} TEllipseParameterUnion;
//
//typedef struct  
//{
//	double endPoint[8];                        // End point coordinates
//	short plane;		                       // Ellipse plane selection, 0: XY; 1: YZ; 2: ZX
//	short dir;			                       // Ellipse direction, 0: clockwise; 1: counterclockwise
//	short overrideSelect;	                   // Velocity override selection, 0: group 1 override; 1: group 2 override
//	short mode;			                       // Ellipse mode, currently supported: 0-auxiliary point mode, 1-standard mode
//
//	TEllipseParameterUnion data;	           // Reserved parameters
//} TEllipseParameter;
//
...
////-------------------------------------------------------
//// Function description: ellipse interpolation description parameters, mode: ELLIPSE_MODE_AU_POINT_2D
////plane--------------Ellipse plane selection, INTERPOLATION_CIRCLE_PLAT_XY(0): XY; INTERPOLATION_CIRCLE_PLAT_YZ(1): YZ; INTERPOLATION_CIRCLE_PLAT_ZX(2): ZX
////dir----------------Ellipse direction, 0: clockwise; 1: counterclockwise
////overrideSelect-----Velocity override selection, 0: group 1 override; 1: group 2 override
////pad----------------Placeholder variable, no need to pass in
////endPoint1----------End point coordinate 1, meaning depends on plane. If plane is XY, endPoint1 and pos1 are X coordinates, endPoint2 and pos2 are Y coordinates
////endPoint2----------End point coordinate 2
////pos1---------------Auxiliary point coordinate 1 on the ellipse
////pos2---------------Auxiliary point coordinate 2 on the ellipse
////-------------------------------------------------------
//typedef struct  
//{
//	short plane;		                     
...
//
////-------------------------------------------------------
//// Function description: ellipse interpolation description parameters, mode: ELLIPSE_MODE_STANDARD_2D
////plane--------------Ellipse plane selection, INTERPOLATION_CIRCLE_PLAT_XY(0): XY; INTERPOLATION_CIRCLE_PLAT_YZ(1): YZ; INTERPOLATION_CIRCLE_PLAT_ZX(2): ZX
////dir----------------Ellipse direction, 0: clockwise; 1: counterclockwise
////overrideSelect-----Velocity override selection, 0: group 1 override; 1: group 2 override
////pad----------------Placeholder variable, no need to pass in
////endPoint1----------End point coordinate 1, meaning depends on plane. If plane is XY, endPoint1 is the X coordinate and endPoint2 is the Y coordinate
////endPoint2----------End point coordinate 2
////centerPoint1-------Ellipse center coordinate 1, meaning depends on plane. If plane is XY, centerPoint1 is the X coordinate and centerPoint2 is the Y coordinate
////centerPoint2-------Ellipse center coordinate 2
////theta--------------Ellipse rotation angle, unit: degrees
////a------------------Ellipse major axis
////b------------------Ellipse minor axis; the minor axis must be shorter than the major axis
////-------------------------------------------------------
//typedef struct
//{
//	short plane;		                     
...
//////////////////////////////////////////////////////////////////////////
// Five-axis model downward shift
//////////////////////////////////////////////////////////////////////////
GT_API GT_GetMachineWorkPos(short crd, short mode,double *pMachinePos, double *pWorkPos);

typedef struct
{
	short			mode;							// 0: FIFO mode, 1: Linear mode, 2: equidistant output mode
	short			dimension;					// 1: 1D, 2: 2D
	short			sourceMode;					// 0: encoder; 1: pulse counter
	short			source[8];					// Encoder compare source
	short			outputMode;					/* Output mode: 0: pulse, 1: level, 2: level auto
										toggle (when level mode and output level auto toggles; not affected by posCompareData level)*/
	short			outputCounter;				// Reserved
	unsigned short	outputPulseWidth;	/* Output pulse width, unit is 1 us; invalid in level mode */
	unsigned short	errorBand;			// 2D position compare output error band
	short			reserve1[2];
	double			reserve2[16];
} TPosCompareModeEx;
...
typedef struct
{
	short probeType;			// Capture type
	short probeIndex;			// Capture index
	short latchType;			// Latch type
	short latchIndex;			// Latch index

	short sense;				// Capture level
	short pad[3];
	double filterWidth;			// Minimum capture width
}TSignalDetect;

typedef struct
{
	short status;				// Completion flag, 0: not triggered, 1: counting, 2: counting complete
	short pad[3];
	double latchWidth;			// Latched pulse-width value
}TSignalDetectStatus;
