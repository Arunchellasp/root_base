// ============================================================
// config.h - GoogolTech controller resource and configuration structures.
//
// Use this header when code needs to inspect, build, save, or load controller
// configuration data such as axis resources, IO mappings, watchdog/watch
// settings, and hardware limits. It is normally used together with gts.h.
//
// Use guide:
// 1. Include config.h where TMcConfig, TAxisConfig, or watch structures are needed.
// 2. Include gts.h as well when calling GT_LoadConfig(), GT_SaveConfigToFile(), or watch APIs.
// 3. Load an existing controller configuration through GT_LoadConfig().
// 4. Modify configuration structures only when the hardware mapping is understood.
// 5. Download/upload configuration with the vendor GT_* configuration functions.
// 6. Keep the controller cfg file matched to the real card, IO board, and axis wiring.
// ============================================================
...
typedef struct McConfig
{
    ...
    TDiConfig      gpi[RES_GPI + 8];		// Expanded for GUS product compatibility. du.ws 20191228
...
typedef struct  
{
	short enable;						// Acquisition enable
	short run;							// Acquisition status
	unsigned long time;					// Acquisition count
	unsigned long head;					// Head pointer
	unsigned long threshold;			// Maximum acquisition count

	short interval;						// Acquisition interval
	short mode;							// Acquisition mode
	unsigned short countBeforeEvent;	// Number of samples before event trigger
	unsigned short countAfterEvent;		// Number of samples after event trigger
	unsigned short varCount;			// Number of acquisition variables
	unsigned short eventCount;			// Number of acquisition events
} TWatchInfo;
